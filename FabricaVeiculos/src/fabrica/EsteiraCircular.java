...
public class EsteiraCircular {
    ...
    public int reservarPosicao() throws InterruptedException {
        vagas.acquire();
        mutex.acquire();
        try {
            int pos = indiceInsercao;
            indiceInsercao = (indiceInsercao + 1) % capacidade;
            return pos;
        } finally {
            mutex.release();
    ...
    public void inserirNaPosicao(Veiculo v, int posicao) {
        buffer[posicao] = v;
        itens.release();
    }
